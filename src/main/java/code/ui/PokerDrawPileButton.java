...
import code.patches.HijackExhaustScreenPatch;
...
public class PokerDrawPileButton extends ClickableUIElement {
    ...
    private static final UIStrings uiStrings = CardCrawlGame.languagePack.getUIString(makeID("PokerDrawPileButton"));
    ...
    private static Texture tex = TexLoader.getTexture(makeImagePath("ui/chips01.png"));

    public PokerDrawPileButton() {
        super((Texture) null,
                0f,
                Y_OFF,
                HB_WIDTH,
    ...
    public void render(SpriteBatch sb) {
        if (((TheBalatro)player).PokerDeck.PokerDrawPile != null && (player.chosenClass == TheBalatro.Enums.THE_BALATRO)) {
            if (!AbstractDungeon.overlayMenu.combatDeckPanel.isHidden) {
                ...
                String msg = Integer.toString(((TheBalatro)player).PokerDeck.PokerDrawPile.size());
                sb.setColor(Color.WHITE);
                draw(sb,
