...
import static code.TheBalatroMod.makeImagePath;
import static code.util.Wiz.adp;
import static com.megacrit.cardcrawl.dungeons.AbstractDungeon.CurrentScreen.*;

public class PokerDeckTopPanel extends TopPanelItem {

    private static final float tipYpos = Settings.HEIGHT - (120.0f * Settings.scale);

    public static final String ID = TheBalatroMod.makeID("PokerDeckTopPanel");

    private static final Texture ICON = TexLoader.getTexture(makeImagePath("ui/chips01.png"));
    ...
    public PokerDeckTopPanel() {
        super(ICON, ID);
    }

    @Override
    public boolean isClickable() {
        if (!AbstractDungeon.player.chosenClass.equals(TheBalatro.Enums.THE_BALATRO)) {
            return false;
        }
    ...
    public void render(SpriteBatch sb) {
        if (AbstractDungeon.player.chosenClass == TheBalatro.Enums.THE_BALATRO) {
            render(sb, isClickable() ? Color.WHITE : Color.DARK_GRAY);
            FontHelper.renderFontRightTopAligned(sb, FontHelper.topPanelAmountFont, Integer.toString(((TheBalatro)adp()).PokerDeck.PokerMasterDeck.size()), this.x + 58.0F * Settings.scale, this.y + 25.0F * Settings.scale, Color.WHITE.cpy());
    ...
    private static void open() {
        ...
        for (AbstractCard q : ((TheBalatro)adp()).PokerDeck.PokerMasterDeck.group) {
